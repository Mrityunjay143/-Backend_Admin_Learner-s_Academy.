...
public class DeleteSubjectsAction extends HttpServlet {

    private Log log = LogFactory.getLog(DeleteSubjectsAction.class);
    private static final long serialVersionUID = 1;
    public HttpSession session;


    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        log.info("Entering into the method doPost in class DeleteSubjectsAction");
        ...
        try {
            ...
            subjectsServiceInterface.deleteSubjects(subjects);

            deleteSubjectsResponse.accumulate("success", true);
            deleteSubjectsResponse.accumulate("message", "Successfully Subject Values Deleted");
            response.setContentType("text/json; charset=UTF-8");
            out.print(deleteSubjectsResponse);


        } catch (Exception e) {
            log.error("Error while executing the method doPost in class deleteSubjects " + e.getMessage());
