...
public class DeleteClassesAction extends HttpServlet {

    private Log log = LogFactory.getLog(DeleteClassesAction.class);
    private static final long serialVersionUID = 1;
    public HttpSession session;


    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        log.info("Entering into the method doPost in class DeleteClassesAction");
        ...
        try {
            ...
            classesServiceInterface.deleteClasses(classes);

            deleteClassesResponse.accumulate("success", true);
            deleteClassesResponse.accumulate("message", "Successfully Class Values Deleted");
            response.setContentType("text/json; charset=UTF-8");
            out.print(deleteClassesResponse);


        } catch (Exception e) {
            log.error("Error while executing the method doPost in class deleteClasses " + e.getMessage());
